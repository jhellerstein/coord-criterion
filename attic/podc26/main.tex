...
% - Futures: H_1 \hext H_2 means H_2 is a future of H_1.
...
Prior work has addressed this question at multiple abstraction layers.
Protocol and task theory provide impossibility and hierarchy results for consensus
and atomic commitment.
CAP relates availability under partitions to strong guarantees such as linearizability.
CALM and its successors connect coordination-freedom to monotonicity in declarative programs.
Transactional isolation and invariant-preserving replication provide additional perspectives.
Each captures an important facet of coordination, but does so within a formalism
tailored to a particular computational model, set of execution assumptions, or programming abstraction.
These formalisms are powerful, yet narrow—and typically not directly comparable.

We instead seek a single intrinsic characterization of coordination,
one that depends only on the specification itself rather than on any
particular protocol or computational model. By formulating specifications over Lamport histories—events partially ordered
by happens-before—and a specification-defined notion of observability,
we obtain a sharp boundary theorem that applies uniformly across domains.

The Coordination Criterion states that
\emph{a specification admits a coordination-free implementation if and only if it is future-monotone.}
Future-monotonicity requires that as histories grow, admissible outcomes
refine rather than contradict earlier ones.
When this condition fails, some causally admissible future makes an earlier
outcome incompatible with all outcomes in that future.
To preserve correctness, implementations must suppress such futures.
That suppression is coordination.
When the condition holds, coordination-free implementations exist.

The criterion is both unifying and diagnostic.
It explains classical impossibility results, partition tradeoffs,
monotonicity-based analyses, and isolation boundaries as instances of a single phenomenon:
whether observable outcomes remain compatible as histories grow.
For example, linearizable registers fail the criterion,
while global snapshots satisfy it.
Coordination requirements can therefore be identified at the level of specification,
before algorithm design begins.

% This uniform treatment is possible because we strip the model down to its essentials.
% We fix only a universe of events and represent executions as Lamport histories.
% A specification then says, for each such history, which observable outcomes are allowed,
% and how those outcomes may be refined over time.
% We assume no synchrony, fairness, failure detectors, or liveness guarantees.
% In this minimal asynchronous setting, coordination becomes a simple semantic fact:
% it is the need to suppress otherwise admissible futures in order to preserve correctness.

We focus on safety-style conditions on observable behavior,
abstracting away from termination and recovery.
Within this scope, coordination is intrinsic exactly when
future-monotonicity fails.
...
Before launching into definitions, we give a brief overview of our approach.

\parhead{Outcomes and refinement order.}
At each history $H$, a specification admits a set of observable outcomes
$\Obs(H)$ that an implementation may expose.
A specification also equips outcomes with a partial order $\Ord$,
where $o_1 \Ord o_2$ means that $o_2$ adds detail without contradicting $o_1$.

\parhead{Stuck histories.}
As events occur, a history may grow into a possible future $H_1$.
A history $H_1$ is \emph{stuck} if all its possible outcomes contradict the past:
for every outcome $o' \in \Obs(H_1)$,
there exists some earlier prefix $H_0$ with outcome
$o \in \Obs(H_0)$ such that $o$ contradicts $o'$.
In a stuck history,  no implementation can remain within specification.
% Equivalently, no outcome in $\Obs(H_1)$ is compatible with
% all outcomes previously admissible along the history.

\parhead{Coordination.}
If a specification admits a stuck history as a possible future,
implementations must suppress that future in order to preserve correctness.
Suppressing causally admissible futures is our semantic notion of coordination.

\parhead{Future-monotonicity.}
A specification has no stuck histories precisely when outcomes can always refine as histories grow; that is,
for every history $H$, every future $H'$ of $H$,
and every $o \in \Obs(H)$,
there exists $o' \in \Obs(H')$ with $o \Ord o'$.
This condition is \emph{future-monotonicity}.
The Coordination Criterion shows that a specification admits a
coordination-free implementation iff it is \mbox{future-monotone}.

\subsection*{Contributions}
This paper makes three contributions.
First, we introduce a lean semantic model of distributed specifications. We represent executions as Lamport histories and define a
specification by an event universe together with the observable outcomes it
admits at each history, ordered by refinement.
Within this minimalist asynchronous setting—without synchrony, fairness,
failure detectors, programming models, or execution disciplines—we
derive semantic notions of \emph{consistency}, \emph{future-consistency},
and \emph{coordination} directly from observability.
Coordination emerges as a purely semantic phenomenon: the necessity to
suppress otherwise admissible futures to preserve correctness.

Second, with this model in hand, we then prove the \emph{Coordination Criterion}:
a specification admits a coordination-free implementation
if and only if it is future-monotone.
This gives a necessary and sufficient condition stated entirely at the
level of observable semantics.

Third, we demonstrate the breadth of this characterization.
Classical impossibility theorems, CAP-style tradeoffs,
CALM-style monotonicity analyses, isolation boundaries,
agreement tasks, and invariant-confluence results
all emerge as instances of the same semantic boundary:
failure of future-monotonicity.
...
A \emph{future} preserves a history's events and causal order, adding only causally later events:
...
\begin{definition}[Future]
  ...
  \end{itemize}
  When $H_1 \hext H_2$, we say that $H_2$ is a \emph{future} of $H_1$.
\end{definition}
...
To reason about compatibility of outcomes across futures, we equip
...
observable outcomes across futures.
...
Future-consistency lifts this notion to futures.

\begin{definition}[Future-Consistent Outcome]
  Let $H_1 \in \Hist$ and $o \in \Obs(H_1)$.
  We say $o$ is \mbox{\emph{future-consistent}} if for every future $H'$ of $H_1$
  (i.e., with $H_1 \hext H'$), there exists $o' \in \Obs(H')$
  with $o \Ord o'$.
  Else it is \mbox{\emph{future-inconsistent}}.
\end{definition}

Future-inconsistent outcomes are exactly those that lead to the
``stuck histories'' of Section~\ref{sec:intuition}: if such an outcome is
exposed, some admissible future contains no outcome compatible with the past.
...
futures.
Section~\ref{sec:coordination-criterion} formalizes this as future-monotonicity with
respect to $\Ord$.
...
\begin{definition}[Admissible Histories]
  ...
  The set $\mathcal{A}(H_{\mathit{in}})$ is upward-closed over futures:
...
\begin{definition}[Future-monotonicity]
  A specification $\Spec=(E,\Obs,\Ord)$ is \emph{future-monotone} if for all
...
A failure of future-monotonicity is witnessed by a future-inconsistent outcome—admitted
...
\begin{remark}
  ...
  Under future-monotonicity, extending a history can only refine outcomes
...
\begin{example}[Replicated register revisited]
  ...
  Thus $\Spec_{\mathit{lin}}$ is not future-monotone: it admits an outcome at $H_1$
...
\parhead{Sufficiency.}
Assume the specification $\Spec=(E,\Obs,\Ord)$ is future-monotone.
...
Because the specification is future-monotone, every
...
\par\addvspace{0.25em}
\parhead{Necessity.}
Assume the specification is not future-monotone.
...
Section~\ref{sec:registers} shows that linearizability is not future-monotone,
...
Hence $\Obs_{\textsf{val}}$ refines monotonically over futures.
...
The future relation is causal (via happens-before), not set-theoretic, and
...
exactly when its observable outcomes are not future-monotone.
...
boundary: whether observable commitments remain stable across futures.
...
futures completely characterizes coordination.
In the sense formalized in Appendix~\ref{app:minimality}, the criterion is exact: any strictly finer semantic boundary would correspond to altering the specification rather than strengthening the condition.
...
constraints on admissible futures and availability as
...
\begin{proof}[Proof sketch]
  ...
  $H_1 \hext H_2$.
  Hence $\Obs(H_1) \Ord \Obs(H_2)$, establishing the future-monotonicity of the specification.
  ...
  that witness failure of future-monotonicity of $\Spec$.
\end{proof}
...
The Coordination Criterion identifies future-monotonicity at the level of
observable outcomes.
...
inducing a future-monotone specification.
...
yet the induced specification $\Spec=(E,\Obs,\Ord)$ is future-monotone
...
happens-before.
By the argument above, such outcomes are preserved across futures,
so the specification is future-monotone.
...
Thus atomic snapshot is not future-monotone.

\parhead{Consequence.}
By the Coordination Criterion, a specification admits a coordination-free
implementation iff it is future-monotone.
...
Thus $\Spec_L$ is future-monotone.
...
including serializability and snapshot isolation—are not future-monotone.
There exist histories $H_1 \hext H_2$ and an outcome
...
are not future-monotone and therefore intrinsically require coordination.
...
We recover this boundary directly via future-monotonicity.
...
Because the future relation can only enlarge the set of reachable states,
$
...
$S \Ord S'$.
Therefore $\Spec_I$ is future-monotone.
...
Thus $\Spec_I$ is not future-monotone.
...
iff its observable outcomes are future-monotone.
...
\begin{proof}
  ...
  Because $\Spec$ is future-monotone,
  ...
  \begin{remark}[On the sufficiency construction]
    ...
    computational model.
    It shows only that when a specification is future-monotone,
  ...
  \begin{remark}[Relation to classical impossibility arguments]
    ...
    as failure of future-monotonicity of $\Obs$.
  \end{remark}
...
\begin{proposition}[Semantic Minimality]
  ...
  such that $\Spec$ is future-monotone with respect to
  $\hext$ under $\Ord^*$.
\end{proposition}
...
\begin{proof}[Proof sketch]
  ...
  Thus under $\Ord^*$,
  the specification is future-monotone with respect to $\hext$.
\end{proof}
...
coordination-freedom must already entail future-monotonicity under some
...
future-monotonicity is the common semantic boundary.
...
future-monotonicity of its observable outcomes
