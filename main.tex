...
The possibility function $\Poss(H)$ characterizes which outcomes executions may
still realize under asynchrony and causality.
The observability function $\Obs(H)$ instead captures which outcomes the
specification treats as correct at history $H$ itself.
...
representative instantiations, chosen to mirror the perspectives emphasized in
the introduction.
...
between closely related specifications that are often treated together.
...
Across all applications we study, the same semantic phenomenon recurs:
coordination is intrinsically required exactly when a specification permits an
observable outcome at some prefix history that cannot be preserved—under the
outcome order—along one or more causally admissible continuations.
In such cases, any correct implementation must exclude some admissible histories.
When this does not occur, coordination-free implementations are possible in
principle.

\subsection{CAP}

In our framework, the CAP tradeoff arises directly from the fact that
linearizability is a \emph{non-monotone} specification.
A read outcome that is admissible at a prefix history may be invalidated by a
causally admissible extension, forcing any correct implementation to exclude some
admissible executions.
When combined with a requirement of maximal availability under partitions—i.e.,
not excluding any partition-respecting executions—this non-monotonicity yields the
classical impossibility.

Concretely, consider a replicated register, where observations record
sequences of completed operations and the outcome order is prefix extension
of those sequences.
...
Thus no terminating read under this partition admits a linearization.

This execution pattern witnesses the semantic source of the CAP tradeoff:
linearizability permits an observable outcome at a prefix history
that cannot be extended
along at least one causally admissible continuation.
Any implementation that remains maximally available under partitions must therefore
violate linearizability in some admissible execution.

\paragraph{Corollary: CAP}
In the asynchronous model above, no implementation of a replicated register can
satisfy linearizable read--write consistency while remaining both correct on all
causally consistent histories and maximally available under partitions.

Formal definitions of partition patterns and maximal availability, and their
connection to the above execution pattern, are given in
Appendix~\ref{app:cap-formal}.

\subsection{Snapshots}

Snapshot objects provide a crisp illustration of the Coordination
Criterion’s boundary character.
Although both global snapshot~\cite{chandy1985distributed} and atomic
snapshot~\cite{afek1993atomic} aim to expose a consistent view of distributed
state, the criterion distinguishes them by how their possible outcomes behave
under history extension.
Formal definitions and proofs appear in
Appendix~\ref{sec:additional-applications}.

Global snapshot is \emph{monotone}.
A possible outcome corresponds to a downward-closed cut of the
happens-before relation.
Extending a history can only add events after such a cut, never invalidate it.
Thus any snapshot admissible at a prefix history remains admissible under all
causally consistent extensions, and coordination-free implementations are
possible.

Atomic snapshot, by contrast, is \emph{non-monotone}.
Consider a history $H$ that yields a state $S$ containing a variable $x$.
Two operations are concurrent at $H$: a write that updates $x$ to $x'$, yielding
a new state $S'$, and a snapshot that reports the current state.
At $H$, the specification admits two possible outcomes: reporting snapshot $S$
or reporting snapshot $S'$.
Because atomic snapshot requires a single linearization point for the snapshot
operation, these choices are incompatible: observing snapshot $S$ commits to an
ordering in which the write occurs later, while observing $S'$ commits to the
opposite ordering.
No single causally admissible outcome can be extended to accommodate both
linearization commitments.
Preventing some extensions is therefore unavoidable—precisely our notion of
coordination.
...
To instantiate CALM in our framework, histories are transducer executions,
and observations record the sets of output facts a program may have produced
so far, ordered by set inclusion.
In monotone logic programs, additional input facts (events) can only cause new facts to be added to output sets
(observations). Non-monotone programs, such as those with negation or
aggregation, may need to retract previously derived facts when new input arrives.
Formal definitions and proofs appear in Appendix~\ref{app:calm-formal}.

\paragraph{Summary.}
Across objects, snapshots, and programs, coordination requirements arise from the
same intrinsic source.
Non-monotone specifications admit observations that cannot be preserved across
all causally admissible extensions, forcing the exclusion of some
causal executions.
Monotone specifications avoid this obstruction, admitting
coordination-free and partition-tolerant realizations.

% ================================================================
\section{Related Work}
% ================================================================
We build upon prior connections between coordination and monotonicity in the
CALM line of work~\cite{li2025coordinationfree,ameloot2013relational,ameloot2016weaker,zinn2012weak,
  hellerstein2010calm}, and in CAP-style availability/consistency tradeoffs~\cite{brewer2000towards,gilbert2002cap}.
The original CALM proofs characterize coordination-freedom via monotonicity in
relational transducer models over input
sets~\cite{ameloot2013relational,ameloot2016weaker}.
Li and Lee move from specific models to more abstract problems, defining monotonicity
for input–output specifications ordered by refinement of observable outcomes~\cite{li2025coordinationfree}.
Their formulation relies on
an explicit mapping from executions to an ordered input space with admissible
partitions.
We too adopt a problem-level perspective, but formulate it directly in a
setting of Lamport histories and outcome orders in a standard
asynchronous message-passing model. This
allows the coordination criterion to be naturally instantiated on
...
related move at the problem level.
Their formulation abstracts from specific programming models by treating
problems as functions \(\mathcal{P} : \mathbb{X} \to \mathbb{V}\) between ordered
domains, and characterizes coordination-freedom semantically in terms of
monotonicity into an explicitly ordered codomain, together with robustness
under admissible partitions.

Our Coordination Criterion is closest in spirit to the problem-level formulation
of Li and Lee~\cite{li2025coordinationfree}.
Like their work, we make refinement structure explicit by parameterizing
correctness by an order, rather than assuming a fixed consistency model.
Where Li and Lee define monotonicity of input–output problems over ordered input
domains, we instead take Lamport histories as the domain and define monotonicity
with respect to history extension in a standard asynchronous model.
This shift allows the criterion to be instantiated directly on classical
distributed objects and consistency conditions—such as registers, snapshots,
agreement tasks, and transactional isolation—without translating them into a
specific query language or replicated-object algebra.
The two approaches thus share a common semantic goal, while differing primarily
in the choice of underlying domain and abstraction boundary.
