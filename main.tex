...
admissibility, it remains framed within a relation-to-relation computational
model with fixed input and output structures, rather than treating histories
and observable outcomes themselves as the primary semantic objects.

Li and Lee~\cite{li2025coordinationfree} pursue a complementary abstraction.
They formulate coordination-freedom at the level of ordered input–output problems,
parameterizing correctness by an explicit refinement order on outputs.
Their notion of extension is set-theoretic—growing the input domain.
By contrast, we take Lamport histories as the semantic domain and
analyze extension via happens-before.
The two approaches share a semantic goal, but differ fundamentally
in what it means for a computation to extend:
input-set inclusion versus causal history extension.

The key methodological difference is the choice of semantic domain.
CALM-style results are formulated over relation-to-relation computations
with set-based input extension and, in many cases, specific language
semantics.
Applying those results to other models—shared objects, agreement tasks,
transactional histories, or application invariants—typically requires
``porting'' the problem into the relational setting, along with proof
obligations establishing that the encoding faithfully preserves the
original notion of correctness.

By centering instead on Lamport histories and observable outcomes as
primitive semantic objects, our framework applies directly to
classical distributed objects, computability tasks, isolation models,
and application-level invariants in their native form.
No translation into a particular programming model is required.
The Coordination Criterion therefore provides a uniform semantic
explanation of coordination across domains that were previously
analyzed using distinct formal encodings.
