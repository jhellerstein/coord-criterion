...
The CALM theorem connects coordination-freedom to monotonicity in logic
programs.
...
The Coordination Criterion states that \emph{a distributed specification admits a
...
An observation is consistent if it leaves open at least one causally admissible
way for the execution to continue within the specification.
...
We capture this using a partial order on observable outcomes, where
...
monotone, in which case observations remain consistent under all causally
admissible history extensions.
...
classical theorems, we show how a single semantic condition on specifications
explains when coordination is and is not intrinsically required.
...
Across all applications we study, the same phenomenon recurs: coordination is
necessary exactly when a specification admits an observation at some history
prefix that cannot be extended along one or more causal continuations.
...
linearizability permits an observation at a prefix history that cannot be extended
...
logic programs as specifications, and output sets as observations.
...
for input–output specifications ordered by refinement~\cite{li2025coordinationfree}.
...
Monotonicity is defined relative to an explicit choice of specification,
...
To discuss CAP formally within our semantic framework, we instantiate consistency as
...
To model eventual consistency, we take observations to be \emph{eventual output
  relations}.
Formally, let $O$ be the set of finite or infinite relational instances over the
output schema of the program.
For a history $H$, define $\Obs(H)$ to be the set of output relations that may be
produced at some node in some fair extension of $H$ (or equivalently, the set of
...
A logic program induces a specification
\[
  \Spec : \Hist \to \mathcal{P}(O)
\]
mapping each history to the set of output relations admissible under the program’s
semantics.
Correctness requires that any relation produced by an implementation belong to
$\Spec(H)$ for the corresponding history $H$.

Under this interpretation, a specification is \emph{monotone} in our sense if and
only if extending a history cannot invalidate a previously admissible output
...
because observations are taken to be eventual outputs ordered by inclusion.
