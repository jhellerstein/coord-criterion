...
\begin{abstract}
  When is coordination \emph{intrinsically required} by a distributed specification, rather than imposed by a particular protocol or implementation strategy? We give a general answer using minimal assumptions. In an asynchronous message-passing model, we represent executions as Lamport histories: partially ordered collections of events ordered by happens-before. We abstract away from implementation mechanics and reason only about the observable outcomes that a specification admits at each history. We show that a specification admits a coordination-free implementation if and only if observable outcomes evolve monotonically as histories grow.

  This \emph{Coordination Criterion} is stated entirely at the level of specifications, independent of any particular programming language, object implementation, or protocol structure. It yields a sharp boundary between specifications that can be implemented without coordination and those for which coordination is unavoidable. The criterion provides a uniform explanation for a range of classical results, including CAP-style impossibility, CALM-style coordination-freedom, agreement and snapshot tasks, transactional isolation levels, and invariant confluence. All are instances of the same underlying semantic phenomenon.
\end{abstract}
...
By formulating specifications directly over Lamport histories—partially ordered
executions under
happens-before—and a
...
This formulation subsumes settings previously studied in isolation:
...
become invalid.
An implementation is coordination-free precisely when it allows all causally admissible
extensions to occur.
...
history with more events—and making \emph{semantic commitments} that
restrict which history extensions are permitted.
As a history grows, learning can make some outcomes no longer realizable.
For example, once $f{+}1$ votes have arrived, outcomes that depend on observing
only $f$ votes are not realizable: no extension of the current history contains
fewer than $f{+}1$ votes.
All causally admissible extensions remain permitted throughout.
Coordination arises only when correctness requires \emph{preventing} outcomes that
\emph{remain causally possible}—for instance, committing to an exact count while
additional increment events remain possible.
In this sense, counting votes is learning; fixing membership is a semantic
commitment. Only the latter constitutes coordination in our sense.
% ================================================================
\section{Computational Model}
% ================================================================
We model executions using Lamport histories, which represent distributed executions as finite or infinite partially ordered events rather than linear traces.
...
We take a minimal semantic view of specifications:
a specification constrains which observable outcomes may occur, abstracting
away from the particular executions that produce them.
Accordingly, we identify two histories as equivalent whenever they admit the
same observable outcomes. This abstraction is captured by the \emph{possibility function} $\Poss$.
For each history $H$, $\Poss(H)$ records the outcomes realized by \emph{some}
admissible extension of $H$.
By focusing on potential outcomes rather than concrete executions,
$\Poss$ isolates the semantic choices that matter for coordination.
In Section~\ref{sec:specs-and-coordination} we will define implementations as
mechanisms that realize a specification by selecting particular history
extensions (Definition~\ref{def:implementation}).
...
Taken together, $\Poss(H)$ describes what could still happen from history $H$,
while $\Obs(H)$ describes what is already safe to expose at $H$.
...
\begin{definition}[Admissible Histories]
  ...
  For a given input history prefix $H_{\mathit{in}}$, the set of
  \emph{admissible histories} is
  $
    \mathcal{A}(H_{\mathit{in}})
    \triangleq
    \{\, H \in \Hist \mid H_{\mathit{in}} \hext \In(H) \,\}.
  $
  These are exactly the histories permitted by asynchrony and causality alone.
  The set $\mathcal{A}(H_{\mathit{in}})$ is upward-closed under history extension.
\end{definition}

\begin{definition}[Coordination-Free Implementation]
  Fix a specification $\Spec = (\Poss, \Obs, \Ord)$.
  ...
  \begin{enumerate}[nosep]
    ...
    \item \emph{(No additional pruning)}
          ...
          where $\mathcal{A}(H_{\mathit{in}})$ denotes the set of admissible
          histories (Definition~\ref{def:admissible}).
  \end{enumerate}
\end{definition}
\noindent
...
\begin{example}[Mechanisms that Preserve vs. Restrict Observations]
  ...
  Under our asynchronous model, retries \emph{preserve} the possibility set $\Poss(H)$:
...
this non-monotonicity: $o_1$ is future-inconsistent, since
for some admissible extension $H_2$ with $H_1 \hext H_2$,
$o_1$ contradicts every outcome in $\Obs(H_2)$.
Fix an input history whose external inputs match $H_1$; then both $H_1$ and
$H_2$ are admissible extensions of that input history.
...
If $\{A\}$ is exposed, there exists an admissible extension in which only $\{B\}$
remains admissible (e.g., after additional evidence favors $B$), yielding an
observation not ordered below the earlier one.
