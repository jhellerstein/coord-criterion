...
Each captures an important facet of coordination, but within a formalism tailored
to a particular mechanism or computational model.

This paper gives a uniform semantic answer.
We show that coordination is a property of a specification itself,
independent of protocol design, programming language, or consistency model.
By formulating specifications over Lamport histories—events partially ordered
by happens-before—and a specification-defined notion of observability,
we obtain a sharp boundary theorem that applies uniformly across settings.
This Coordination Criterion (Theorem~\ref{thm:coordination-criterion}) states that \emph{a specification admits a coordination-free implementation if and only if it is future-monotone.}
...
Our formulation assumes only an asynchronous message-passing model with
well-formed Lamport histories.
We impose no synchrony, failure detectors, fairness, or operational constraints.
A specification is characterized solely by which observable outcomes it admits at each history.
Coordination is defined semantically: the necessity to rule out causally admissible futures in order to preserve correctness.
...
Before launching into definitions, we give an informal overview of our approach.

\parhead{Outcomes and refinement.}
...
there exists some earlier prefix $H_0$ and some outcome
...
coordination-free implementation if and only if it is future-monotone.
