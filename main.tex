...
expose any admissible outcome at a history, without ruling out any
outcome that remains possible under admissible extensions of that history.
Necessity shows that if monotonicity fails, coordination is required:
implementations must rule out a specification-permitted outcome that remains
...
% \noindent
% Equivalently: coordination is required exactly when correctness forces the
% implementation to rule out a specification-permitted outcome that remains possible under
% admissible extensions of the same history.
...
and for each $H \in \mathcal{A}(H_{\mathit{in}})$ choose
...
Thus exposing $o$ at $H$ does not rule out any outcome
admitted along an admissible extension.
...
$
we have $H_1,H_2 \in \mathcal{A}(\In(H_1))$ and
$o_1 \in \Poss_{\In(H_1)}(H_1)$.

Suppose, for contradiction, that there exists a coordination-free
implementation $I$ for input prefix $\In(H_1)$.
Then
$
  \Poss^{I}_{\In(H_1)}(H_1)
  =
...
requires that some outcome in $\Obs(H_2)$ remain possible.
But none are compatible with $o_1$.
Thus
$
  \Poss^{I}_{\In(H_1)}(H_1)
  \subsetneq
...
Once a value is exposed, every admissible extension is compatible with it.
...
forcing implementations to avoid realizing causally admissible histories to preserve correctness.

The power of this minimalist criterion lies in its reuse.
Across diverse case studies (\S\ref{sec:applications}, Appendix~\ref{sec:additional-applications}),
the same semantic test isolates a single obstruction—non-monotonicity—behind
coordination requirements.
Rather than reproving existing results, it provides a uniform diagnostic
for specifications across disparate literatures.

Seen through this lens, familiar results align.
The Coordination Criterion identifies the semantic boundary:
when a specification intrinsically requires coordination.
CAP captures the operational consequence under partition once that boundary is crossed.
CALM characterizes the same boundary more narrowly, within declarative programming models.

Crucially, the Coordination Criterion is a precise diagnostic.
Monotonicity is defined relative to an explicit specification
$\Spec = (E,\Obs,\Ord)$: an event universe $E$, an observability
function $\Obs$, and an outcome order $\Ord$.
Any refinement sufficient to recover coordination-freedom can be
expressed by adjusting this semantic triple; beyond that boundary,
...
therefore rule out some causally consistent executions and introduce
coordination.
...
\begin{proof}
  ...
  \begin{remark}[On the idealized sufficiency construction]
    ...
    only from the need to avoid realizing certain causally consistent executions—and separates this
  ...
  fail to realize at least one of the causally consistent extensions $H_1$ or $H_2$.
  Equivalently,
...
\begin{proof}[Proof sketch]
  ...
  Because $I$ realizes every causally admissible extension, extending a
