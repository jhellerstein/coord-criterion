...
As a concrete illustration, we provide a history extension
for our register example in Appendix~\ref{ex:history-extension}.
...
Informally,
% following the standard asynchronous message-passing model in
% distributed computing,
an implementation consists of a collection of (possibly
...
for any of its extensions. In particular, extending a history cannot invalidate
\emph{any} previously allowed observation; it is not enough that some allowed
...
no further steps. We assume that $\Ord$ is a partial order on observable outcomes and that
$\Spec : \Hist \to \mathcal{P}(O)$ is total.
No progress or fairness assumptions (such as termination or wait-freedom) are required.
...
$o \in \Spec(H)$, but makes no commitment to that choice: reporting an observation
does not constrain future behavior.
...
This execution-exclusion perspective is also studied operationally by
Baccaert et al.~\cite{baccaert2023distributed}.

\paragraph{Tightness.}
Prior CALM-style work has identified weaker notions of monotonicity that suffice
for coordination-freedom within particular semantic models
\cite{ameloot2016weaker,zinn2012weak}.
In our framework, monotonicity is defined relative to an explicit choice of
$(\Spec,\Obs,\Ord)$, and refinements of prior results correspond to different
choices of this semantic interface rather than to weakening the criterion itself.
In this sense, the Coordination Criterion is sharp at the level of specifications:
there is no strictly weaker semantic condition that characterizes
coordination-freedom uniformly across all choices of $(\Spec,\Obs,\Ord)$.
...
and use this semantic restriction to construct a CAP-style witness of unavailability.
...
Under such a witness, any implementation that completes the pending operation
in a partition-respecting execution necessarily produces an observation that is
not admissible under linearizability.
