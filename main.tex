...
\begin{abstract}
  ...
  This \emph{Coordination Criterion} is stated directly over
  Lamport histories and the observable outcomes defined by a specification,
  and is independent of any particular programming language,
...
While $\Poss(H)$ describes what executions may still realize, $\Obs(H)$ describes
which of those outcomes the specification permits to be observable at history $H$.
...
Monotonicity captures the intuition that extending a history cannot invalidate
outcomes.
Every outcome in $\Obs(H_1)$ for a prefix history can be extended to a
$\Ord$-compatible outcome in $\Obs(H_2)$ for any extension $H_1 \hext H_2$.
Monotonicity constrains only the evolution of observable
outcomes ($\Obs$); it places no
restriction on which outcomes remain possible under history extension ($\Poss$).

Equivalently, monotonicity here rules out \emph{semantic regret}:
extending a history may refine the set $\Obs(H)$,
but it cannot make a previously observable outcome inconsistent with all outcomes
in $\Obs(H')$ for the extension\footnote{Formally, this condition is stronger than monotonicity under the lower
...
discarding a previously observable outcome corresponds exactly to pruning a
...
familiar distributed objects and programming models. Rather than reproving
classical theorems, we show how
a single semantic condition on specifications—monotonicity of observable
outcomes under history extension—explains when coordination is and is not
intrinsically required.
CAP and CALM serve as core instantiations; further applications—snapshots,
agreement tasks, transactional isolation levels, and invariant
confluence—are deferred to Appendix~\ref{sec:additional-applications}.

\paragraph{Unifying pattern.}
Across all applications we study, the same phenomenon recurs: coordination
is necessary exactly when a specification admits an observable outcome
at some prefix history that cannot be extended, under the outcome order,
along one or more causally admissible continuations.
Non-monotone specifications force implementations to exclude admissible histories
to remain correct; monotone specifications do not.

\subsection{CAP}

In our framework, the CAP tradeoff arises because linearizability is a
\emph{non-monotone} specification.
...
Concretely, consider a replicated register.
...
In contrast to the read/write CAP setting, CALM is application-level;
among distributed declarative logic programs, the monotone programs are exactly those
that admit coordination-free distributed evaluation~\cite{ameloot2013relational}.
To instantiate CALM in our framework, the mapping is direct:
transducer executions as histories,
logic programs as specifications,
possible output sets as $\Poss(H)$,
and program-defined outputs as observable outcomes.
...
\paragraph{Summary.}
Taken together, the CAP and CALM instantiations show that coordination requirements
across seemingly disparate settings stem from the same semantic source.
Non-monotone specifications force implementations to exclude causally admissible
histories, while monotone specifications admit coordination-free,
partition-tolerant realizations.
...
Our Coordination Criterion is closest in spirit to this problem-level view.
Like Li and Lee, we make refinement structure explicit by parameterizing
correctness by an outcome order rather than assuming a fixed notion of
consistency.
Where we differ is in the underlying domain and notion of extension:
instead of input sets ordered by inclusion, we work directly over Lamport
histories ordered by causal extension in a standard asynchronous
message-passing model.
This allows monotonicity—and hence coordination-freedom—to be evaluated
directly over history-to-observation specifications
\(\Obs : \Hist \to \mathcal{P}(O)\), without committing to a particular query
language, replicated-object algebra, or clause calculus.
...
Crucially, Gallifrey requires programmers to specify \emph{the order with respect
