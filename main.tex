...
transducers~\cite{hellerstein2010calm,ameloot2013relational,ameloot2016weaker},
...
By contrast, our work makes only the assumptions common to all distributed systems.
...
refinement relation.
This minimal formulation accommodates arbitrary input and output
structures and any program or machine model, yielding a general diagnostic
criterion for when coordination is \emph{intrinsic to a specification itself}.
Because the criterion relies only on causality and semantic refinement—rather than
language properties, system models, or execution assumptions—it determines
when coordination is unavoidable without appeal to exogenous assumptions.
...
behavior.
Most recently, Baccaert and Ketsman~\cite{baccaert2026spectrum} move beyond
language-specific analyses by modeling distributed computations as
\emph{semantic mappings} between relations, without committing to a concrete
syntax (e.g., Datalog or SQL).
They parameterize coordination-freedom by a family of \emph{system constraints},
each restricting which executions are admissible, and show that different
constraint classes induce a spectrum of monotonicity conditions.
While this approach abstracts away from particular programming languages, it
retains a mapping-based view of computation with relations as inputs and outputs.

Li and Lee~\cite{li2025coordinationfree} pursue a complementary abstraction.
Rather than varying system constraints within a fixed semantic domain, they
formulate coordination-freedom directly at the level of ordered input--output
specifications, independent of any particular programming model.
Their approach likewise abstracts away from relational transducers, but still
assumes a fixed notion of execution admissibility and partitioning.

Despite differences in system model and semantic domain, the role played by both
constraint classes~\cite{baccaert2026spectrum} and admissible partitions~\cite{li2025coordinationfree}
...
Under the assumption of relations as inputs and outputs,
the results of~\cite{baccaert2026spectrum} can thus be understood as instantiations
of our framework with parameterized possibility spaces.
Li and Lee’s admissibility conditions similarly correspond to restricting the
space of possible outcomes, though defined over abstract ordered domains rather
than relational semantics.

Our Coordination Criterion differs in both scope and abstraction boundary.
Rather than fixing a programming model, semantic mapping, or system constraint,
we take Lamport histories as the semantic domain and allow arbitrary outcome
structures and refinement orders.
This isolates coordination as a purely semantic obstruction, independent of
programming formalism or constraint regime.
...
criterion applies directly to classical PODC-style objects, tasks, and consistency
conditions—including CAP-style registers, snapshots, agreement tasks, and
transactional guarantees—without translation into a specific language or object
model (cf.~Section~\ref{sec:applications} and
Appendix~\ref{sec:additional-applications}).
While unifying the full constraint spectrum of~\cite{baccaert2026spectrum} with
general Lamport histories and arbitrary outcome orders appears natural, we leave
that development to future work.

We also note a connection to Gallifrey~\cite{milano2023gallifrey}, which makes
