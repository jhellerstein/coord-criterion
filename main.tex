...
Intuitively, consistency means that an observation made under partial information
(e.g., during a partition) can never \emph{force} a contradiction.
...
One might expect a specification to be defined by the set of causal extensions
admissible from each history.
...
The possibility function $\Poss$ is the induced abstraction that records, for each history $H$, which outcomes are realized by \emph{some} admissible extension.
Making admissible history extensions explicit would not change our results, but would
complicate the presentation without adding semantic insight.
...
extensions---and hence outcomes---under asynchrony.
...
still realize in causally admissible extensions of a history.
However, a specification may choose to treat only some of these outcomes as
correct or observable at history $H$ itself.
This separation allows us to reason explicitly about the relationship between
future behaviors and present commitments.
