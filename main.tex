...
In our setting, consistency is a property of \emph{observations}—that is, of what a
system reveals about an execution so far.
An observation is consistent if it leaves open at least one possible extension of
the history that remains within the specification.
This notion is intentionally weak, requiring only the absence of semantic dead ends.
Intuitively, consistency supports a \emph{no-regret} principle:
an observation made under partial information (e.g., during a partition) should
never have to be retracted or invalidated by a causally admissible extension of
the history.
...
monotone, so observable outcomes remain consistent under all history extensions.
...
\begin{definition}[Outcome Order and Contradiction]
  ...
  \emph{outcome order}.
  Intuitively, $o_1 \Ord o_2$ means that $o_2$ is a compatible extension or
  refinement of $o_1$.
  Two observations are said to \emph{contradict} if they have no common extension
  in this order, that is, if there is no outcome $o$ such that
  $o_1 \Ord o$ and $o_2 \Ord o$.
\end{definition}

The outcome order $\Ord$ does not impose an execution order or a notion of program
state.
It captures \emph{semantic compatibility under history extension}: an outcome
$o_2$ extends $o_1$ when the commitments represented by $o_1$ remain valid after
those represented by $o_2$.
Two outcomes contradict when no future behavior can satisfy both.
This allows the framework to express semantic incompatibility without appealing to
operational mechanisms such as linearization points, schedules, or traces.
...
The role of consistency in our framework is to formalize the \emph{no-regret}
...
% In the next section, we package these components into a formal notion of
% specification and coordination; we then formalize this requirement as a
% monotonicity condition on specifications with respect to the outcome order
% $\Ord$ in Section~\ref{sec:monotonicity}.

% At this point our framework diverges from the CALM line of work~\cite{ameloot2013relational,ameloot2016weaker}.
% Those results characterize coordination-freedom only after a specification
...
% More generally, outcomes need not resemble executions or traces:
% $\Poss$ may record abstract semantic state—such as values, summaries, or sets of
% visible facts—ordered by refinement or inclusion.
...
\cite{hellerstein2010calm,ameloot2013relational,ameloot2016weaker}.
