...
\begin{definition}[Observations]
  ...
  Intuitively, \(\Obs(H)\) is the set of outcomes that may be exposed at the
  chosen interface after history \(H\).
\end{definition}
...
As histories are extended, the set $\Obs(H)$ may change: some outcomes that were
admissible for a prefix history may cease to be admissible once additional events
occur.
...
later.
To make this notion precise, we compare outcomes using an order that captures
semantic compatibility under extension.
...
and implementation formalisms out of the semantic core. Specifications are introduced
...
\begin{definition}[Coordination-Free Implementation]
  An implementation is \emph{coordination-free} if correctness never requires it
  to exclude any execution that is permitted by asynchrony and causality alone.
  ...
  Membership in $\mathcal{A}(H_{\mathit{in}})$ is called
  \emph{causal admissibility}:
  it requires only that the external inputs extend $H_{\mathit{in}}$ and that the
  history respect Lamport’s happens-before relation.
  It is unrelated to any storage-level or object-level consistency model.
  ...
  \begin{enumerate}[nosep]
    \item
          ...
          $ \emph{(Observational correctness)}
    \item
          $
            \mathcal{R}_I(H_{\mathit{in}}) = \mathcal{A}(H_{\mathit{in}}).
          $ \emph{(Causal completeness / no additional pruning)}
  \end{enumerate}
\end{definition}
\noindent
Causal completeness captures the absence of coordination:
...
\begin{proof}
  ...
  some outcome $o_H \in \Spec(H)$ to expose at history $H$.
