...
% - Observation function: \Obs : \Hist \to O.
% - Specification: \Spec : \Hist \to \mathcal{P}(O).
...
waiting or stronger primitives~\cite{herlihy1991waitfree,herlihy1999topological}.
...
consistency requirement under all admissible asynchronous executions; it is
deliberately agnostic about progress guarantees such as termination or
wait-freedom. Accordingly, the Coordination Criterion focuses on when coordination is
semantically unavoidable, not on how non-monotonicity is resolved.
...
\begin{definition}[Observation Function]
  An \emph{observation function} is a mapping
  \[
    \Obs : \Hist \rightarrow O,
  \]
  where \(O\) is a set of observable outcomes.
  An outcome \(o \in O\) is a summary of a history \(H = (E,\rightarrow)\) that
  represents all information visible at the chosen interface.
\end{definition}
...
\begin{remark}
  In many applications there is a natural choice of outcome order,
  such as prefix extension on execution logs, refinement on abstract states, or
  set inclusion on visible facts.
  Coarser orders are also possible, but they generally make the Coordination
  Criterion less informative, so choosing \(\Obs\) and \(\Ord\) is a key part of the
  modeling task.
  Extreme choices can trivialize the criterion in either direction: for
  instance, if all outcomes are incomparable or all histories share the same
  observation, then every specification is vacuously monotone, whereas an
  overly fine-grained observation or order can make every interesting
  specification non-monotone.
  Similarly, if $O$ contains a greatest element $\top$ that extends every
  outcome (a ``anything can happen'' summary), then no two observations ever
  contradict in our order-theoretic sense, and the notion of consistency
  degenerates; choosing $\Obs$ and $\Ord$ so that semantically incompatible
  observations genuinely lack a common extension is therefore an essential part
  of faithful modeling.
  We return to these modeling choices in examples throughout
  Section~\ref{sec:applications} and Appendix~\ref{sec:additional-applications}.
\end{remark}
...
\begin{remark}[Faithful Observations]
  ...
  The observation function $\Obs$ must faithfully reflect the semantic obligations
...
\begin{definition}[Coordination-Free (Causally Complete) Implementation]
  Fix a specification $\Spec$ and the asynchronous model above.
  ...
  \begin{enumerate}[nosep]
    \item (Correctness) for all $H \in \mathcal{R}_I(H_{\mathit{in}})$, the
          observation $\Obs(H)$ is an element of $\Spec(H)$; and
...
Informally, an implementation consists of a collection of (possibly
...
admits for a prefix history can be extended to a $\Ord$-compatible observation for any
of its extensions.
...
The proof proceeds in two directions, establishing sufficiency and necessity of
monotonicity for coordination-free implementations.
$H_{\mathit{in}}$.

\paragraph{Sufficiency.}
For the ``if'' direction, we construct an ideal implementation $I_\Spec$ whose
state is the current history and that does not introduce any additional
restrictions on admissible executions beyond causal consistency.
From any admissible input history $H_{\mathit{in}}$, the implementation allows
all causally consistent extensions (the set $\mathcal{A}(H_{\mathit{in}})$) and, at each configuration $H$, reports some
observation $\Obs(H) \in \Spec(H)$.
By construction these observations are always permitted for the current
history, and monotonicity of $\Spec$ ensures that extending the history never
forces previously admissible observations to be revoked.
Thus correctness holds and $I_\Spec$ realizes exactly the causally consistent
extensions in $\mathcal{A}(H_{\mathit{in}})$ for each input history.
This construction is intentionally idealized and should be read purely as a
semantic existence proof, not as a practical algorithm.

\paragraph{Necessity.}
For the ``only if'' direction, assume $\Spec$ is non-monotone.
Then there exist $H_1 \hext H_2$ such that some observations in $\Spec(H_1)$
and $\Spec(H_2)$ contradict under $\Ord$.
We choose an input history $H_{\mathit{in}}$ whose external inputs are exactly
those of $H_1$, so both $H_1$ and $H_2$ are causally consistent extensions of
$H_{\mathit{in}}$.
If a correct implementation $I$ were coordination-free, it would have to
realize both histories from $H_{\mathit{in}}$.
However, along any run leading to $H_2$ there is a prefix whose history is
exactly $H_1$; at that configuration, any observation chosen to be compatible
with $\Spec(H_1)$ will necessarily be incompatible with $\Spec(H_2)$ on some
admissible extension.
To remain correct on all admissible executions, such an implementation must
therefore exclude at least one of $H_1$ or $H_2$ from its realizable
histories, contradicting the no-pruning clause of coordination-freedom.
Hence no correct implementation of a non-monotone $\Spec$ can be
coordination-free.\hfill$\square$

\paragraph{Tightness.}
Prior work has proposed refinements of monotonicity to enlarge the class of
coordination-free programs within particular semantic models.
Notably, follow-on work to CALM identified weaker forms of monotonicity that
remain sufficient in specific transducer and logic-based settings
\cite{ameloot2016weaker,zinn2012weak}.
Such refinements arise because monotonicity is evaluated relative to a fixed
semantic representation.

The Coordination Criterion is sharp at the level of specifications.
Because monotonicity is defined relative to an explicit choice of specification,
observation function, and outcome order, any such refinement can be expressed as
a different $(\Spec,\Obs,\Ord)$ triple rather than as a weaker semantic condition.
There is therefore no strictly weaker semantic characterization of
coordination-freedom than the Coordination Criterion.
...
To relate this semantic notion of coordination to the CAP tradeoff, we now
apply the definitions above to construct a CAP-style witness of
unavailability.
Specifically, we express network partitions as constraints on which history
extensions are operationally reachable.

A network partition does not change the semantic space of asynchronous
histories; instead, it rules out certain history extensions that deliver
messages across the partition.
Formally, fix a history $H_i$ and a partition pattern $P$ that forbids
delivery on a subset of channels from some cut of $H_i$ onward, and let
$\mathsf{Ext}_P(H_i)$ denote the set of causally consistent extensions $H$ with
$H_i \hext H$ that respect this restriction.

We say that $(H_i,P)$ is a \emph{witness of unavailability} if every history
in $\mathsf{Ext}_P(H_i)$ violates the linearizable read--write specification.
Under such a witness, remaining available means that an implementation must
produce a completed outcome for the pending operation in some
partition-respecting execution.
However, by assumption, every such execution yields an observation that is not
admissible under linearizability.
Thus any implementation that remains available under $P$ must violate the
specification, yielding a contradiction.

For the replicated-register running example, let $H_i$ contain a write of value
$1$ at process $p$ followed, after a partition, by a read at process $q$, and let
...
Returning the initial value would require linearizing the read before the write,
violating real-time order, while returning $1$ would require a linearization in
which the write is visible to the read, asserting a causal relationship that no
history respecting $P$ can realize.
...
We have shown that, in our asynchronous semantic model, a distributed
specification admits a coordination-free consistent implementation if and only
if it is monotone with respect to history extension under the chosen outcome
order.
Instantiations of this Coordination Criterion recover the semantic content of
classical CAP-style tradeoffs, CALM-style program monotonicity, snapshot
protocols, agreement tasks, and transactional and invariant-preserving
semantics.

Our CAP and CALM corollaries make precise how CALM refines CAP.
Non-monotone specifications, such as linearizable registers, necessarily require
``pick two'' sacrifices of consistency or availability under partitions, whereas
monotone specifications admit coordination-free implementations that are both
partition-tolerant and maximally available in the sense of
Definition~\ref{def:maximal-availability}.
The CALM Theorem is one instance of this phenomenon, characterizing exactly those
relational transducer programs that fall within this coordination-free region.

More generally, the Coordination Criterion is sharp at the semantic level.
Because monotonicity is defined relative to an explicit choice of specification,
...
In this sense, the Coordination Criterion marks a minimal boundary within this
semantic framework: beyond it, coordination becomes semantically unavoidable,
independent of the choice of programming model or implementation formalism.

The Coordination Criterion characterizes whether coordination is required, but
not how much coordination is required.
Some specifications inherently demand multiple, sequential coordination phases,
in which admissible observations depend on progressively stronger forms of
common knowledge.
A canonical example is Paxos: before the agreement condition can be satisfied,
processes must first establish sufficient shared context (e.g., via leader
election or quorum intersection), and only then emit decisions that satisfy the
specification.
These cases point toward a hierarchy of coordination depth and a semantic theory
of multi-round coordination.

Our focus in this paper has been on safety-style semantic conditions on observable
behavior.
Orthogonal work on coordination-free liveness and free termination
\cite{power2025freetermination} studies when progress or termination can be
guaranteed without exogenous coordination.
Taken together, these directions suggest a broader semantic account of
coordination as a resource—governing not only what outcomes are permitted, but
also when and how systems may safely commit to them.
...
Thanks to Shulu Li and Edward Lee for helpful discussions.
...
The observation function $\Obs$ maps a history to the set of reachable database
states, ordered by set inclusion: $o_1 \Ord o_2$ if every state in $o_1$ also
appears in $o_2$.
Given an application invariant $I$, the specification $\Spec_I$ selects those
observations in which all visible states satisfy $I$.
...
Accordingly, the induced specification $\Spec_I$ is monotone with respect to
history extension and, by the Coordination Criterion, admits coordination-free
implementations.
If $I$ is not $I$-confluent, there exist histories $H_1 \hext H_2$ such that an
observation admissible under $\Spec_I(H_1)$ is inconsistent with every admissible
observation under $\Spec_I(H_2)$.
...
\begin{proof}
  ...
  that, for all $H \in \mathcal{R}_{I_\Spec}(H_{\mathit{in}})$, the observation
  $\Obs(H)$ is compatible with $\Spec(H)$ and
  ...
  For each configuration $H$, the implementation's client-visible observation is
  chosen to be some $\Obs(H) \in \Spec(H)$; monotonicity of $\Spec$ ensures that
  every such observation remains $\Ord$-compatible with at least one admissible
  observation of every extension of $H$.

  \emph{Correctness.}
  By definition of $\Obs$, for every admissible input history $H_{\mathit{in}}$
  and every realizable history $H \in \mathcal{R}_{I_\Spec}(H_{\mathit{in}})$ we
  have $\Obs(H) \in \Spec(H)$, so clause~(i) of coordination-freedom holds.
...
\begin{proposition}[Semantic Minimality]
  ...
  Then for each $\Spec \in \mathcal{C}$ there exists a choice of observation
  function $\Obs$ and outcome order $\Ord$ for which $\Spec$ is monotone with
  respect to history extension under $\Ord$.
\end{proposition}
...
\begin{proof}[Proof sketch]
  ...
  Define an observation function $\Obs_I$ that summarizes the observable behavior
  of $I$ on all prefixes of $H$, ordered by set inclusion.
  Because $I$ does not prune any causally consistent extension, extending a
  history can only add observations produced by $I$, never remove them.
  Thus the induced specification $\Spec_I(H) = \Obs_I(H)$ is monotone under
  history extension.

  Finally, since $I$ is correct by assumption, $\Spec_I$ is observationally
  equivalent to the original specification $\Spec$ with respect to the chosen
  observation function and order.
  Hence $\Spec$ is monotone for the triple $(\Spec,\Obs_I,\Ord)$.
\end{proof}

\paragraph{Discussion.}
The observation function constructed above is intentionally coarse and is used
only to establish semantic minimality.
It shows that any refinement of monotonicity sufficient for coordination-freedom
can be expressed as a change to the $(\Spec,\Obs,\Ord)$ interface rather than as a
weaker coordination criterion.
