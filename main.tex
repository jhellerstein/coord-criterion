...
coordination within specific computational abstractions and execution models
that refine the canonical asynchronous setting
\cite{hellerstein2010declarative,ameloot2013relational,ameloot2015weaker}.

Baccaert and Ketsman~\cite{baccaert2026spectrum} generalize this perspective
by modeling distributed computation as relation-to-relation mappings
parameterized by system constraints.
These constraints restrict which executions are admissible,
yielding a spectrum of monotonicity conditions under progressively stronger
assumptions.
The semantic domain, however, remains fixed to structured inputs and outputs,
with extension defined by set inclusion over input relations.

Li and Lee~\cite{li2025coordinationfree} develop a related theory
in a replicated-object architecture.
Their model decomposes implementations into a coordination function
and a replicated object, with executions built from write and merge steps.
Coordination-freedom is characterized relative to replica consistency,
and extension is defined by growing the input set.
Within this setting they obtain a CALM-style equivalence between
monotonicity and coordination-free replica consistency.
...
define specifications directly in terms of admissible histories and
observable outcomes ordered by refinement.
History extension is causal (via happens-before), not set-theoretic.
Replication and merge semantics arise as special cases,
but are not assumed.

This distinction is substantive.
Most of our case studies—linearizable registers, agreement tasks,
snapshot specifications, isolation levels, and invariant confluence—
do not concern replica consistency at all.
They analyze non-monotonicity in shared objects,
membership commitments, ordering constraints, and transactional semantics.
By centering histories and observable outcomes rather than a particular
programming or storage model, the Coordination Criterion applies uniformly
across replicated and non-replicated settings.
Monotonicity of observable outcomes under causal extension
is the common semantic boundary in each case.
