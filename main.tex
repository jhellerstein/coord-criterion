...
\begin{example}[Possibility vs. observability]
  ...
  history and outcomes that are immediately observable—will be central to this paper.
\end{example}
...
We illustrate the Coordination Criterion through a sequence of specifications,
showing how coordination pressure arises from mutable state, replication under
partition, and agreement mechanisms that control observability.
We conclude with CALM, where the same semantic condition governs coordination in
declarative programs.

\paragraph{Unifying pattern.}
Across examples, coordination is required exactly when specifications are non-monotone: when there exist histories
$H_1 \hext H_2$ and an outcome $o \in \Obs(H_1)$ such that no outcome in
$\Obs(H_2)$ extends $o$ under $\Ord$.
Such an outcome is \emph{future-inconsistent}.
By the Coordination Criterion, this semantic non-monotonicity rules out
